...
import { Search, Star, CheckCircle, Zap } from 'lucide-react'

export default function Marketers() {
  const [search, setSearch] = useState('')
  const [nicheFilter, setNicheFilter] = useState('All')
  const [sortBy, setSortBy] = useState('revenue')
  const { user, mirrorAffiliate } = useAuth()
  const { addToast } = useToast()

  const all = getAllAffiliates()
  const niches = ['All', ...new Set(all.map(a => a.niche))]
  ...
  const filtered = useMemo(() => {
    ...
    return result
  }, [all, search, nicheFilter, sortBy])

  const handleMirror = (aff) => {
    if (!user) { addToast('Please sign in to mirror marketers', 'warning'); return }
    const result = mirrorAffiliate(aff.id, aff.minDeposit)
    if (result.success) addToast(`Now mirroring ${aff.name}`, 'success')
    else addToast(result.error, 'error')
  }

  const isMirrored = (affId) => user?.mirroredAffiliates?.includes(affId)

  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-white">Marketer Directory</h1>
        <p className="text-slate-400 mt-1">{all.length} verified affiliate marketers ready to mirror</p>
      </div>
      ...
      <div className="glass-card p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="input-field sm:w-40">
            ...
            <option value="followers">Audience</option>
          ...
          <div key={aff.id} className="glass-card-hover p-5 group">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              ...
              <div className="text-center p-2 bg-white/5 rounded-lg">
                ...
                <p className="text-[10px] text-slate-400 uppercase tracking-wide">Audience</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Link to={`/marketer/${aff.id}`} className="px-3 py-1.5 text-xs font-medium text-slate-300 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                  View Profile
                </Link>
                {isMirrored(aff.id) ? (
                  <span className="px-3 py-1.5 text-xs font-medium text-emerald-400 bg-emerald-500/10 rounded-lg flex items-center gap-1">
                    <Zap className="w-3 h-3" /> Mirroring
                  </span>
                ) : (
                  <button onClick={() => handleMirror(aff)} className="btn-primary text-xs px-3 py-1.5">
                    Mirror
                  </button>
                )}
              </div>
