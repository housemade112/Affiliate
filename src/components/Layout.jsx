...
export default function Layout() {
  ...
  const SidebarContent = () => (
    <div className={`h-full flex flex-col w-64 transition-colors ${isDark ? 'bg-[#1A1D21] border-r border-white/5' : 'bg-[#005645] border-r border-emerald-800/40'}`}>
      {/* Logo */}
      <div className="h-20 flex items-center px-6 border-b border-emerald-800/30">
      ...
      <div className="flex-1 py-8 px-4 space-y-1 overflow-y-auto">
        <p className="px-4 text-[10px] font-extrabold text-white/30 uppercase tracking-widest mb-4">MY HI</p>
        {NAV_ITEMS.map(item => (
          <Link key={item.path} to={item.path}
            ...
            <item.icon className={`w-5 h-5 ${isActive(item.path) ? 'text-[#C3F53C]' : 'text-white/40'}`} />
            {item.label}
          </Link>
        ))}
      </div>
      ...
      {user && (
        <div className={`p-4 border-t ${isDark ? 'border-white/5' : 'border-emerald-800/30'}`}>
          <Link to="/profile"
            className="flex items-center gap-3 p-3 rounded-2xl hover:bg-white/5 transition-colors mb-3">
            <div className="w-10 h-10 rounded-full bg-emerald-700/50 border border-emerald-500/30 flex items-center justify-center text-white font-extrabold text-sm shadow-sm">
              {user.name?.split(' ').map(n => n[0]).join('')}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-extrabold text-white truncate">{user.name}</p>
              <p className="text-[10px] text-[#C3F53C]/80 font-mono uppercase tracking-widest">Verified</p>
            </div>
          </Link>
          <button onClick={() => { logout(); navigate('/') }}
            className="w-full flex items-center justify-center gap-2 py-2.5 text-xs font-bold text-rose-400 hover:bg-rose-500/10 rounded-xl transition-colors">
        ...
        <header className={`sticky top-0 z-30 flex items-center justify-between h-16 px-4 sm:px-8 border-b transition-colors ${
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2">
              ...
              <span className={`text-sm font-extrabold ${isDark ? 'text-white' : 'text-slate-900'}`}>{currentNav.label}</span>
            </div>
          </div>
                ...
                <div className={`absolute right-0 top-12 w-80 rounded-2xl shadow-2xl border overflow-hidden z-50 ${
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.length === 0 ? (
                      <p className={`p-8 text-center text-sm font-medium ${isDark ? 'text-white/40' : 'text-slate-400'}`}>No notifications</p>
                    ) : notifications.map(n => (
                      <button key={n.id} onClick={() => markRead(n.id)}
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`text-sm font-extrabold ${isDark ? 'text-white' : 'text-slate-900'} flex items-center gap-1.5`}>
                            {n.title}
